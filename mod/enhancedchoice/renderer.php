...
class mod_enhancedchoice_renderer extends plugin_renderer_base {

    /**
     * Returns HTML to display enhancedchoices of option
    ...
    public function display_options($options, $coursemoduleid, $vertical = false, $multiple = false) {
        ...
        $target = new moodle_url('/mod/enhancedchoice/view.php');
        ...
        $html .= html_writer::start_tag('ul', array('class' => 'enhancedchoices list-unstyled unstyled'));

        $availableoption = count($options['options']);
        $enhancedchoicecount = 0;
        foreach ($options['options'] as $option) {
            ...
            $option->attributes->id = 'enhancedchoice_'.$enhancedchoicecount;
            $option->attributes->class = 'mx-1';

            $labeltext = $option->text;
            if (!empty($option->attributes->disabled)) {
                $labeltext .= ' ' . get_string('full', 'enhancedchoice');
                $availableoption--;
            }
        ...
        $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'action', 'value'=>'makeenhancedchoice'));
        $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'id', 'value'=>$coursemoduleid));

        if (empty($options['previewonly'])) {
            if (!empty($options['hascapability']) && ($options['hascapability'])) {
                if ($availableoption < 1) {
                    $html .= html_writer::tag('label', get_string('enhancedchoicefull', 'enhancedchoice'));
                } else {
                    $html .= html_writer::empty_tag('input', array(
                        'type' => 'submit',
                        'value' => get_string('savemyenhancedchoice', 'enhancedchoice'),
                ...
                if (!empty($options['allowupdate']) && ($options['allowupdate'])) {
                    $url = new moodle_url('view.php',
                            array('id' => $coursemoduleid, 'action' => 'delenhancedchoice', 'sesskey' => sesskey()));
                    $html .= html_writer::link($url, get_string('removemyenhancedchoice', 'enhancedchoice'), array('class' => 'ml-1'));
                }
            } else {
                $html .= html_writer::tag('label', get_string('havetologin', 'enhancedchoice'));
            }
        }
    ...
    /**
     * Returns HTML to display enhancedchoices result
     * @param object $enhancedchoices
     * @param bool $forcepublish
     * @return string
     */
    public function display_result($enhancedchoices, $forcepublish = false) {
        if (empty($forcepublish)) { //allow the publish setting to be overridden
            $forcepublish = $enhancedchoices->publish;
        }

        $displaylayout = $enhancedchoices->display;

        if ($forcepublish) {  //ENHANCEDCHOICE_PUBLISH_NAMES
            return $this->display_publish_name_vertical($enhancedchoices);
        } else {
            return $this->display_publish_anonymous($enhancedchoices, $displaylayout);
        }
    }
     ...
     * @return string
     */
    public function display_publish_name_vertical($enhancedchoices) {
        $html ='';
        $html .= html_writer::tag('h3',format_string(get_string("responses", "enhancedchoice")));

        $attributes = array('method'=>'POST');
        $attributes['action'] = new moodle_url($this->page->url);
        $attributes['id'] = 'attemptsform';

        if ($enhancedchoices->viewresponsecapability) {
            $html .= html_writer::start_tag('form', $attributes);
            $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'id', 'value'=> $enhancedchoices->coursemoduleid));
            $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'sesskey', 'value'=> sesskey()));
        ...
        $table->summary = get_string('responsesto', 'enhancedchoice', format_string($enhancedchoices->name));
        $table->data = array();

        $count = 0;
        ksort($enhancedchoices->options);
        ...
        $accessiblecell->text = get_string('enhancedchoiceoptions', 'enhancedchoice');
        ...
        $usernumberheader->text = get_string('numberofuser', 'enhancedchoice');
        $columns['usernumber'][] = $usernumberheader;

        $optionsnames = [];
        foreach ($enhancedchoices->options as $optionid => $options) {
            $celloption = clone($celldefault);
            $cellusernumber = clone($celldefault);

            if ($enhancedchoices->showunanswered && $optionid == 0) {
                $headertitle = get_string('notanswered', 'enhancedchoice');
            } else if ($optionid > 0) {
                $headertitle = format_string($enhancedchoices->options[$optionid]->text);
            }
            $celltext = $headertitle;

            // Render select/deselect all checkbox for this option.
            if ($enhancedchoices->viewresponsecapability && $enhancedchoices->deleterepsonsecapability) {
                ...
                $selectalltext = get_string('selectalloption', 'enhancedchoice', $headertitle);
                $deselectalltext = get_string('deselectalloption', 'enhancedchoice', $headertitle);
        ...
        // This extra cell is needed in order to support accessibility for screenreader. MDL-30816
        $accessiblecell = new html_table_cell();
        $accessiblecell->text = get_string('userchoosethisoption', 'enhancedchoice');
            ...
            if ($enhancedchoices->showunanswered || $optionid > 0) {
                if (!empty($options->user)) {
                    ...
                    foreach ($options->user as $user) {
                        ...
                        $userfullname = fullname($user, $enhancedchoices->fullnamecapability);
                        $checkbox = '';
                        if ($enhancedchoices->viewresponsecapability && $enhancedchoices->deleterepsonsecapability) {
                        ...
                        $userimage = $this->output->user_picture($user, array('courseid' => $enhancedchoices->courseid, 'link' => false));
                        $profileurl = new moodle_url('/user/view.php', array('id' => $user->id, 'course' => $enhancedchoices->courseid));
        ...
        if ($enhancedchoices->viewresponsecapability && $enhancedchoices->deleterepsonsecapability) {
            ...
            foreach ($enhancedchoices->options as $optionid => $option) {
                if ($optionid > 0) {
                    $actionoptions['choose_'.$optionid] = get_string('chooseoption', 'enhancedchoice', $option->text);
                }
            }
            $selectattributes = [
                'data-action' => 'toggle',
            ...
            $selectnothing = ['' => get_string('chooseaction', 'enhancedchoice')];
            $select = new single_select($actionurl, 'action', $actionoptions, null, $selectnothing, 'attemptsform');
            $select->set_label(get_string('withselected', 'enhancedchoice'));
     ...
     * @return string
     */
    public function display_publish_anonymous_horizontal($enhancedchoices) {
        debugging(__FUNCTION__.'() is deprecated. Please use mod_enhancedchoice_renderer::display_publish_anonymous() instead.',
                DEBUG_DEVELOPER);
        return $this->display_publish_anonymous($enhancedchoices, ENHANCEDCHOICE_DISPLAY_VERTICAL);
    }
     ...
     * @return string
     */
    public function display_publish_anonymous_vertical($enhancedchoices) {
        debugging(__FUNCTION__.'() is deprecated. Please use mod_enhancedchoice_renderer::display_publish_anonymous() instead.',
                DEBUG_DEVELOPER);
        return $this->display_publish_anonymous($enhancedchoices, ENHANCEDCHOICE_DISPLAY_HORIZONTAL);
    }

    /**
     * Generate the enhancedchoice result chart.
     *
     * Can be displayed either in the vertical or horizontal position.
     *
     * @param stdClass $enhancedchoices EnhancedChoices responses object.
     * @param int $displaylayout The constants DISPLAY_HORIZONTAL_LAYOUT or DISPLAY_VERTICAL_LAYOUT.
     * @return string the rendered chart.
     */
    public function display_publish_anonymous($enhancedchoices, $displaylayout) {
        $count = 0;
        $data = [];
        $numberofuser = 0;
        $percentageamount = 0;
        foreach ($enhancedchoices->options as $optionid => $option) {
            ...
            if($enhancedchoices->numberofuser > 0) {
                $percentageamount = ((float)$numberofuser / (float)$enhancedchoices->numberofuser) * 100.0;
            }
        ...
        $series = new \core\chart_series(format_string(get_string("responses", "enhancedchoice")), $data['series']);
