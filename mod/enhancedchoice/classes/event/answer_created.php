...
/**
 * The mod_enhancedchoice answer created event.
 *
...
/**
 * The mod_enhancedchoice answer created event class.
 *
...
class answer_created extends \core\event\base {
     ...
     *
     * @param stdClass $enhancedchoiceanswer record from 'enhancedchoice_answers' table
     * @param stdClass $enhancedchoice record from 'enhancedchoice' table
     * @param stdClass $cm record from 'course_modules' table
     * @param stdClass $course
     * @return self
     */
    public static function create_from_object($enhancedchoiceanswer, $enhancedchoice, $cm, $course) {
        global $USER;
        $eventdata = array();
        $eventdata['objectid'] = $enhancedchoiceanswer->id;
        $eventdata['context'] = \context_module::instance($cm->id);
        $eventdata['userid'] = $USER->id;
        $eventdata['courseid'] = $course->id;
        $eventdata['relateduserid'] = $enhancedchoiceanswer->userid;
        $eventdata['other'] = array();
        $eventdata['other']['enhancedchoiceid'] = $enhancedchoice->id;
        $eventdata['other']['optionid'] = $enhancedchoiceanswer->optionid;
        $event = self::create($eventdata);
        $event->add_record_snapshot('course', $course);
        $event->add_record_snapshot('course_modules', $cm);
        $event->add_record_snapshot('enhancedchoice', $enhancedchoice);
        $event->add_record_snapshot('enhancedchoice_answers', $enhancedchoiceanswer);
        return $event;
    }
    ...
    public static function get_name() {
        return get_string('eventanswercreated', 'mod_enhancedchoice');
    }
    ...
    public static function get_objectid_mapping() {
        return array('db' => 'enhancedchoice_answers', 'restore' => 'answer');
    }
